...
protected:
  ...
  _DecodeEx decodeEx(const _Exec &execComb,const _MemFetch1 &mem1);
  _Exec execute(uint32_t extMemStall);

  _State::_RegBlocking blockCtrl(const _Decode &decodeComb) const;
  _StallCtrl stallCtrl(uint32_t stallFetch,uint32_t stallDec,uint32_t stallDecEx,uint32_t stallExec,uint32_t condExec,uint32_t flushPipeline1,uint32_t flushPipeline2);
