...
MTEST(testOperationsBasic, test_that_mac_after_unary_negation_does_not_hang)
{
  // regression: a unary NEG immediately before mac() used to leave the arith unit's
  // activeOp_ permanently "busy" (nothing ever clears it for CMD_MAC), stalling forever.
  // uses a fixed cycle budget instead of executeUntilAddr() so a regression fails loudly
  ...
  tester.expectSymbol("r", qfp32_t(3.0 * 5.0 - 4.0 * 6.0));
}
