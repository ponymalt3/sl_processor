...
public:
  RTProgTester(RTProg prog) : prog_(prog), s_(prog_), codeGen_(s_, 16), parser_(codeGen_)
  {
    ...
    proc_.getExternalMemory().setFaultHandler([&](uint32_t addr, uint32_t data, bool write) {
      ...
      if(!result.first)
      {
        throw Memory::FaultException(addr,
                                     write ? Memory::FaultException::Write : Memory::FaultException::Read);
      }
      return result.second;
    });
  }

  Error parse(uint32_t reserveParameter = 0,
              bool generateFullEntryVector = false,
              uint32_t inlineFunctionThreshold = 0)
  {
  ...
  uint16_t getCodeAt(uint32_t addr) { return codeGen_.getCodeAt(addr); }

  uint32_t getCodeSize() const { return codeGen_.getCurCodeAddr(); }
  ...
  LoadAndSimulateProcessor& getProcessor() { return proc_; }
  ...
  void expectMemoryAt(qfp32_t addr, qfp32_t data) { proc_.expectThatMemIs(addr, data); }
