...
public:
  ...
  void expect(uint32_t addr, uint32_t mustBeLessEqual, uint32_t mustBeGreaterEqual)
  {
    if(file_.is_open())
    {
      file_ << "0010" << " " << std::setw(8) << (addr) << " " << std::setw(8) << (mustBeLessEqual) << " "
...
public:
  ...
  void writeMemory(qfp32_t addr, uint32_t value) { writeMemory((int32_t)addr, value); }

  void writeMemory(qfp32_t addr, qfp32_t value) { writeMemory(addr, value.getAsRawUint()); }

  void writeMemory(uint32_t addr, double value) { writeMemory((int32_t)addr, qfp32_t(value)); }

  void writeMemory(uint32_t addr, qfp32_t value) { writeMemory((int32_t)addr, value.getAsRawUint()); }
  ...
  void expect(qfp32_t expectedValue, uint32_t addr, const std::string& expectString = "")
  {
    ...
    mtest::manager::instance() = mtest::condition(expectedValue == value, ss.str(), mtest::condition::expect);
  }
