...
void SLArithUnit::reset()
{
  ...
  activeOp_ = 0;
  newOpAdded_ = false;
  mac_lane_raw_[0] = _qfp32_t(0).toRaw();
  mac_lane_raw_[1] = _qfp32_t(0).toRaw();
  mac_term_count_ = 0;
}
...
void SLArithUnit::addOperation(const _DecodeEx& decEx)
{
  ...
  if(activeOp_ == 0)
  {
    if(decEx.cmd_ != SLCode::CMD_MAC && decEx.cmd_ != SLCode::CMD_INVALID)
    {
  ...
  if(en)
  {
    switch(decEx.cmd_)
    {
      ...
      case SLCode::CMD_SHFT:
        ...
        break;
      case SLCode::CMD_MAC: {
        uint32_t lane = mac_term_count_ & 1;
        mac_lane_raw_[lane] = (_qfp32_t::initFromRaw(mac_lane_raw_[lane]) + a * b).toRaw();
        ++mac_term_count_;
      }
      break;
      case SLCode::CMD_MAC_RES:
        pipeline_[(curCycle_ + 3) % 32].result_ =
            (_qfp32_t::initFromRaw(mac_lane_raw_[0]) + _qfp32_t::initFromRaw(mac_lane_raw_[1])).toRaw();
