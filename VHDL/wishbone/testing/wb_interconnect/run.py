#!/usr/bin/env python3
"""Cocotb simulation runner for wb_interconnect tests (invoked by CMake)."""
...
VHDL_ROOT          = SCRIPT_DIR.parent.parent.parent          # wb_interconnect/ → testing/ → wishbone/ → VHDL/
REPO_ROOT          = VHDL_ROOT.parent
COCOTB_LIB         = VHDL_ROOT / "cocotb"
_DEFAULT_BUILD_DIR = REPO_ROOT / "build" / "sim" / "sim_wb_interconnect"
...
def run(modules: list, build_dir: Path, waves: bool = False):
    ...
    runner.test(
        ...
        plusargs=GHDL_PLUSARGS + ([f"--fst={build_dir}/wb_ixs_wrapper.fst"] if waves else []),
        waves=False,
    )
