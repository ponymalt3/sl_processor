...
begin  -- rtl
  ...
  slave_o.ack   <= '1' when state = FSM_ENABLE else '0';
  slave_o.err   <= '0';
  slave_o.stall <= not idle;
  ...
  begin  -- process
    if reset_n_i = '0' then             -- asynchronous reset (active low)
      ...
      if state = FSM_SETUP then
        ...
        state <= FSM_ENABLE;
        --din_mux <= active;
      elsif slave_i.stb = '1' and slave_i.cyc = '1' and active /= SLV_NONE then
        state <= FSM_SETUP;
        din_mux <= active;
        apb_we_o <= sig.we;
        apb_dout_o <= sig.dout;
   ...
   process (active, active_din, apb_din_i, din_mux, dout_reg, slave_i, state)
     variable addr : std_ulogic_vector(15 downto 0);
  begin  -- process

    addr := std_ulogic_vector(slave_i.adr(15 downto 0));
    sig <= ('0', (others => '0'), '0', (others => '0'), (others => '0'));

    if active /= SLV_NONE then
      sig.we <= slave_i.we;
      sig.dout <= slave_i.dat;
      sig.addr <= To_StdUlogicVector(std_logic_vector(unsigned(addr)-unsigned(Peripherals(active).low)));
      sig.sel(active) <= '1';
    end if;

    slave_o.dat <= (31 downto 8 => '0') & dout_reg;
    if state = FSM_ENABLE then
      slave_o.dat <= (31 downto 8 => '0') & active_din;
    end if;
    ...
    for i in 0 to Peripherals'high loop
      if addr >= Peripherals(i).low and addr <= Peripherals(i).high then
        active <= i;
      end if;
    end loop;  -- i
  ...
  idle <= '1' when state = FSM_IDLE or state = FSM_ENABLE else '0';
