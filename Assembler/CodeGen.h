...
struct _Instr
{
  ...
  uint32_t getIrsOffset() const { return SLCode::IRS::getOffset(code_); }
  ...
  bool isLoadAddr() const { return (code_ ^ SLCode::Load::Code) >> (16 - SLCode::Load::Bits) == 0; }

  bool isLoopInstr() const { return (code_ ^ SLCode::Loop::Code) >> (16 - SLCode::Loop::Bits) == 0; }
...
{
public:
  virtual void instrOperation(const _Operand& opa,
                              const _Operand& opb,
                              uint32_t op,
                              TmpStorage& tmpStorage) = 0;
  ...
  virtual void instrCompare(const _Operand& opa,
                            const _Operand& opb,
                            uint32_t cmpMode,
                            uint32_t execMode,
                            bool negate,
...
public:
  ...
  void instrGoto2();
  void instrCompare(const _Operand& opa,
                    const _Operand& opb,
                    uint32_t cmpMode,
                    uint32_t execMode,
                    bool negate,
  ...
  uint32_t getCurCodeAddr() const { return codeAddr_; }
  ...
  uint16_t getCodeAt(uint32_t addr) { return instrs_[addr].code_; }
  ...
  uint32_t findSymbolAsLink(const Stream::String& symbol) { return symbolMaps_.top().findSymbol(symbol); }
