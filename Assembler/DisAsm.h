...
public:
  ...
  static std::vector<std::string> getLinesFromCode(const uint16_t* code, uint32_t size)
  {
    ...
    for(uint32_t i = 0; i < size; ++i)
    {
      ...
      // checked before movInstrToString: its MOVDATA2(2) match (0xFF80) ignores bit 6,
      // which is what actually distinguishes it from this (correctly 10-bit-masked) family
      if((s = signalWaitInstrToString(code[i])) != "invalid")
      {
        result.push_back(s);
        continue;
      }
      ...
      if((s = loadInstrToString(
              code[i], ((i + 1) < size) ? code[i + 1] : 0, ((i + 2) < size) ? code[i + 2] : 0)) != "invalid")
      {
        result.push_back(s);
        continue;
      }
  ...
  static std::string opInstrToString(uint16_t code)
  {
    if((code & 0x8000) == 0)
    {
      ...
      return std::string("result = ") + muxAToString(code & 2, code & 1, code & 0x800) + " " +
             opToString(extCmd + ((code >> 12) & 7)) + " [IRS+" + irsOffsetToString(code) + "]";
    }
    ...
    if((code & 0xF000) == 0xC000)
    {
      ...
      return std::string("result = ") + muxAToString(code & 2, (code >> 8) & 1, incA) + " " +
             opToString(((code >> 5) & 0xF)) + " " + muxBToString(code & 8, (code >> 2) & 1, incB);
    }
  ...
  static std::string signalWaitInstrToString(uint16_t code)
  {
    if((code & 0xFFC0) != SLCode::SignalWait::Code)
    {
      return "invalid";
    }

    if(code & (1 << (2 + SLCode::SignalWait::BUS_LOCK)))
      return "bus lock";
    if(code & (1 << (2 + SLCode::SignalWait::BUS_UNLOCK)))
      return "bus unlock";
    if(code & (1 << (2 + SLCode::SignalWait::WAIT_ALL)))
      return "wait all";
    if(code & (1 << (2 + SLCode::SignalWait::WAIT_SIG)))
      return "wait signal";
