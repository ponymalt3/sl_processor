...
CodeGen::CodeGenDelegate::CodeGenDelegate(CodeGen& codeGen, Label& target)
    : codeGen_(codeGen), target_(target)
{
...
void CodeGen::instrOperation(const _Operand& opa, const _Operand& opb, uint32_t op, TmpStorage& tmpStorage)
{
  ...
  if(((!aIsResult && bIsResult) || (aIsIRS && !bIsIRS)) && op != '/' && op != '-' &&
     op != SLCode::CMD_SHFT)  // div/sub/shft
  {
    swap(a, b);
  ...
  writeCode(
      SLCode::Op::create(
          translateOperand(a), translateOperand(b), translateOperation(op), irsOffset, aIncAddr, bIncAddr),
      symRef);
}
...
void CodeGen::instrMov(const _Operand& opa, const _Operand& opb)
{
  ...
  Error::expect(b.type_ != _Operand::TY_IR_ADDR0 && b.type_ != _Operand::TY_IR_ADDR1 &&
                b.type_ != _Operand::TY_IR_IRS)
      << stream_ << "cant read ad0/ad1/irs";
  ...
  if(b.type_ == _Operand::TY_VALUE || b.isArrayBaseAddr())
  {
    ...
    if(b.isArrayBaseAddr())
    {
      ...
      Error::expect(getCurCodeAddr() > 0 && instrs_[getCurCodeAddr() - 1].isIrsInstr())
          << "internal error" << ErrorHandler::FATAL;
...
void CodeGen::instrUnaryOp(const _Operand& opa, uint32_t unaryOp)
{
  ...
  Error::expect(unaryOp == SLCode::UNARY_LOG2 || unaryOp == SLCode::UNARY_NEG ||
                unaryOp == SLCode::UNARY_TRUNC)
      << "unary op internal error";
...
void CodeGen::instrGoto(const Label& label)
{
  ...
  {
    // backward jumping gotos are only generated by RTAsm loops therefore it is not necessary to mark goto
    // instr as loop complex
...
void CodeGen::instrCompare(const _Operand& opa,
                           const _Operand& opb,
                           uint32_t cmpMode,
                           uint32_t execMode,
                           bool negate,
...
void CodeGen::instrLock(bool unlock)
{
  if(unlock)
  {
    ...
    while(i > 0)
    {
      ...
      Error::expect(instrs_[i].isLoopInstr() == false) << "loops inside bus lock are not allowed";
      Error::expect(instrs_[i].writesAD1() == false) << "a1 must not be written inside a bus lock block";
    ...
    Error::expect((getCurCodeAddr() - 1 - i) < 32)
        << "no more than 32 instr inside bus lock block are allowed";
...
uint32_t CodeGen::addStructDeclaration(const Stream::String& name, const std::vector<Stream::String>& fields)
{
  ...
  Error::expect(structTypeIds_.find(key) == structTypeIds_.end())
      << stream_ << "struct '" << name << "' already defined" << ErrorHandler::FATAL;
  Error::expect(fields.size() > 0) << stream_ << "struct '" << name << "' must have at least one field"
                                   << ErrorHandler::FATAL;
...
{
  Error::expect(typeId < structTypes_.size())
      << "internal error: invalid struct type id" << ErrorHandler::FATAL;
...
{
  uint32_t symRef = symbolMaps_.top().findSymbol(name);
  Error::expect(symRef != SymbolMap::InvalidLink)
      << stream_ << "internal error: symbol '" << name << "' not found" << ErrorHandler::FATAL;
...
{
  Error::expect(loopDepth_ < MaxLoopDepth)
      << "internal error: no more loop frames available" << ErrorHandler::FATAL;
...
void CodeGen::storageAllocationPass(uint32_t size, uint32_t numParams)
{
  ...
  for(uint32_t i = symbolMaps_.top().getStartAddr(); i < codeAddr_; ++i)
  {
    ...
    while(arrayAllocInfo_.size() > 0 && arrayAllocInfo_.begin()->first == i)
    {
      ...
      symInf.allocatedAddr_ =
          allocator.allocate(symInf.allocatedSize_, symInf.flagsAllocateHighest_, symInf.flagIsArray_);
...
CodeGen::_FunctionInfo& CodeGen::addFunctionAtCurrentAddr(const Stream::String& symbol)
{
  ...
  _FunctionInfo function{new SymbolMap(stream_, getCurCodeAddr()),
                         ...
                         0,
                         false,
                         {0, std::vector<_Instr>(), std::vector<bool>()}};
...
{
  Error::expect(entryVectorSize_ >= numberOfEntries * entrySizeInInstrs)
      << "Entry vector size too small" << ErrorHandler::FATAL;
...
_Operand CodeGen::resolveOperand(const _Operand& op, bool createSymIfNotExists)
{
  if(op.type_ == _Operand::TY_SYMBOL)
  {
    ...
    Error::expect(symRef != SymbolMap::InvalidLink)
        << "symbol " << name << " not found" << ErrorHandler::FATAL;
    ...
    {
      // make sure that array space is allocated before a certain point in code => allocated now when it is
      // declared arrayAllocInfo_
  ...
  {
    Error::expect(loopDepth_ != 0 && op.loopIndex_ < loopDepth_)
        << "using loop index '" << char('i' + op.loopIndex_) << "' outside loop" << ErrorHandler::FATAL;
...
SLCode::Operand CodeGen::translateOperand(_Operand op)
{
  switch(op.type_)
  {
    case _Operand::TY_RESULT:
      return SLCode::Operand::REG_RES;
    case _Operand::TY_IR_ADDR0:
      return SLCode::Operand::REG_AD0;
    case _Operand::TY_IR_ADDR1:
      return SLCode::Operand::REG_AD1;
    case _Operand::TY_IR_IRS:
      return SLCode::Operand::REG_IRS;
    case _Operand::TY_RESOLVED_SYM:
      return SLCode::Operand::IRS;
...
SLCode::Command CodeGen::translateOperation(char op)
{
  ...
  {
    case '+':
      return SLCode::Command::CMD_ADD;
    case '-':
      return SLCode::Command::CMD_SUB;
    case '*':
      return SLCode::Command::CMD_MUL;
    case '/':
      return SLCode::Command::CMD_DIV;
    default:
...
{
  return symbolMaps_.top().createSymbolNoToken(1, true);  // for calls alloc high but why for everything
                                                          // else??
}
...
{
  Error::expect(codeAddr_ < sizeof(instrs_) / sizeof(instrs_[0]))
      << "code buffer full" << ErrorHandler::FATAL;
  ...
  if(ref < RefLabelOffset && instrs_[codeAddr_].isLoadAddr())
  {
    ...
    symbolMaps_.top()[ref].updateLastAccess(
        0xFFFFFFFF);  // dont release cause we dont know when data is referenced last
  }
...
void CodeGen::moveCodeBlock(uint32_t startAddr, uint32_t size, uint32_t targetAddr)
{
  ...
  Error::expect(size <= sizeof(instrs_) / sizeof(instrs_[0]) - getCurCodeAddr())
      << "not enough instr buffer space" << ErrorHandler::FATAL;
...
void CodeGen::rebaseCode(uint32_t startAddr, uint32_t endAddr, int32_t offset)
{
  ...
  for(int32_t i = startAddr; i <= endAddr;)
  {
    ...
    if(instrs_[i].isLoadAddr() && instrs_[i].symRef_ == RefLoad)
    {
      ...
      qfp32_t v = _Instr::restoreValueFromLoad(
          instrs_[i], load2 ? instrs_[i + 1] : _Instr({0, 0}), load3 ? instrs_[i + 2] : _Instr({0, 0}));
